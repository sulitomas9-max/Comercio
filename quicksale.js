...
async function qsFinalizarVenta() {
  ...
  if (store.payMethod === 'cash') {
    const recv = parseFloat(document.getElementById('qs-cash-recv')?.value) || 0;
    if (recv < total) { toast('Monto insuficiente', 'err'); return; }
  }

  await processSale();   // reutiliza función de caja.js

  // Resetear panel de cobro rápido
  const inp = document.getElementById('qs-cash-recv');
  if (inp) inp.value = '';
  qsCalcChange();
  renderQSCart();
