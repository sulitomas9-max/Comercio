...
function renderUserGrid() {
  ...
  grid.innerHTML = store.users.map(u => `
    <div class="user-card ${store.selectedUserId === u.id ? 'sel' : ''}" id="uc-${u.id}" data-uid="${u.id}">
      <div class="ico">${u.avatar}</div>
      <div class="nm">${u.name}</div>
      <div class="rl">${u.role === 'admin' ? 'Administrador' : 'Cajero'}</div>
    </div>
  `).join('');
}
...
(function init() {
  ...
  if (grid) grid.addEventListener('click', e => {
    ...
    if (!card) return;
    const id = card.dataset?.uid || card.id.replace('uc-', '');
    if (id) selectUser(id);
  });
