...
async function cerrarCaja() {
  ...
  Object.assign(store.cajaAbierta, {
    ventasEf, totalRetiros, esperado, contado,
...
function changeQty(cartKey, delta) {
  ...
  if (newQty < 1) { store.cart = store.cart.filter(c => (c._cartKey || c._comboId || String(c.id)) !== cartKey); renderCart(); return; }
...
function setQty(cartKey, val) {
  ...
  if (!item.isCombo) {
    const prod = store.products.find(p => p.id === item.id);
    const unidades = item.unidades || 1;
    if (prod && newQty * unidades > prod.stock) { toast(`Stock insuficiente (máx ${Math.floor(prod.stock/unidades)})`,'err'); newQty = Math.floor(prod.stock/unidades); }
  }
  item.qty = newQty;
  renderCart();
}
...
function _aplicarFiltroHistorial() {
  ...
  if (q) ventas = ventas.filter(v => v.items.some(i => i.name.toLowerCase().includes(q)) || (v.userName||'').toLowerCase().includes(q));
  if (desde) { const [dy,dm,dd] = desde.split('-').map(Number); const dd2 = new Date(dy,dm-1,dd); ventas = ventas.filter(v => { const d = parseLocalDate(v.date); return d && d >= dd2; }); }
  if (hasta) { const [hy,hm,hd] = hasta.split('-').map(Number); const hh2 = new Date(hy,hm-1,hd,23,59,59); ventas = ventas.filter(v => { const d = parseLocalDate(v.date); return d && d <= hh2; }); }

  const tbody = document.getElementById('hist-table');
  const empty = document.getElementById('hist-empty');

  if (!ventas.length) { if (tbody) tbody.innerHTML = ''; if (empty) empty.style.display = 'block'; _updateHistMetrics([]); return; }
...
function _updateHistMetrics(ventas) {
  ...
  const mc = {}; validas.forEach(v => { mc[v.method] = (mc[v.method]||0)+1; });
...
function _renderHistorialCajas() {
  ...
  if (!all.length) { tb.innerHTML = ''; if (em) em.style.display = 'block'; return; }
  if (em) em.style.display = 'none';
  tb.innerHTML = [...all].reverse().map(c => {
    const ret = store.retiros.filter(r=>r.cajaId===c.id).reduce((s,r)=>s+r.monto,0);
    const vef = c.ventasEf !== undefined ? c.ventasEf : store.sales.filter(s=>s.cajaId===c.id&&s.method==='cash').reduce((s,v)=>s+v.total,0);
