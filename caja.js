...
function generateTicket(sale) {
  const localName  = window.STORE_CONFIG?.localName || 'MI COMERCIO';
  ...
  document.getElementById('ticket-content').innerHTML = `
    <div class="tk-wrap">
      ...
      <div class="tk-totals">
        ...
        <div class="tk-total-row tk-total-final">
          <span>TOTAL</span>
          <span>${formatMoney(sale.total)}</span>
        </div>
        <div class="tk-total-row" style="color:var(--txt2);font-size:12px">
          <span>Método de pago</span>
          <span>${methodName[sale.method] || sale.method}</span>
        </div>
      </div>
      ...
      <div class="tk-footer">
        ...
        <div style="font-size:10px;margin-top:4px;opacity:.6">Conserve este ticket</div>
      </div>
    </div>`;
...
function exportTicketPDF() {
  // Usar la API de impresión nativa con estilos @media print
  window.print();
}
...
function renderHistory() {
  ...
  if (!store.sales.length) {
    ...
    tb.innerHTML = [...store.sales].reverse().map(v => {
      ...
      const devCount  = (store.devoluciones || []).filter(d => d.saleId === v.id).length;
      return `
        <tr class="${isAnulada ? 'row-anulada' : ''}">
          ...
          <td style="font-weight:700;color:${isAnulada ? 'var(--red)' : 'var(--accent)'}">
            ${isAnulada ? '<s>' : ''}${formatMoney(v.total)}${isAnulada ? '</s>' : ''}
          </td>
          ...
          <td>
            ${isAnulada
              ...
              : devCount > 0
                ? `<span class="badge low">Dev. x${devCount}</span>`
                : '<span class="badge ok">OK</span>'}
          </td>
          <td>
            <div style="display:flex;gap:4px">
              <button class="btn sm" onclick="verTicket(${v.id})">🧾</button>
              ${!isAnulada ? `<button class="btn sm" onclick="openDevolucionModal(${v.id})" title="Devolución">↩</button>` : ''}
            </div>
          </td>
        </tr>`;
    }).join('');
...
function _renderHistorialDevoluciones() {
  ...
  tb.innerHTML = [...devs].reverse().map(d => `
    <tr>
      <td style="font-size:12px;color:var(--txt2)">${d.fecha}</td>
      <td style="font-weight:700">#${d.saleId}</td>
      <td><span class="badge ${d.type === 'anulacion' ? 'out' : 'low'}">${d.type === 'anulacion' ? 'Anulación' : 'Parcial'}</span></td>
      <td style="font-size:11px;color:var(--txt2)">${d.items.map(i => `${i.name} x${i.qty}`).join(', ')}</td>
      <td style="font-weight:700;color:var(--red)">${formatMoney(d.total)}</td>
