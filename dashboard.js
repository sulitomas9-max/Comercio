...
function renderHistory() {
  const ventas = [...store.sales].sort((a, b) => b.id - a.id);
  _renderHistorialFiltros(ventas);
  _aplicarFiltroHistorial();
}

function _renderHistorialFiltros(ventas) {
...
function _aplicarFiltroHistorial() {
  ...
  // Métricas filtradas
  const totalFiltrado = ventas.filter(v => !v.anulada).reduce((s, v) => s + v.total, 0);
  const countFiltrado = ventas.filter(v => !v.anulada).length;
