/**
 * firebase.js — BazarHub (con soporte offline)
 * 
 * NUEVO: Cola offline
 * - Si no hay WiFi, las operaciones se guardan en localStorage
 * - Al volver la conexión, se sincronizan automáticamente
 * - El badge de conexión muestra el estado en todo momento
 */
...
// Guarda el estado local de los datos para poder operar sin internet
function saveLocalData() {
  try {
    const snapshot = {
      ...
      orders:      store.orders,
      combos:      store.combos,
      users:       store.users.map(u => ({ ...u, pass: undefined })), // sin passwords
      // IDs
...
function loadLocalData() {
  try {
    ...
    // Solo carga si tiene menos de 7 días
    if (Date.now() - snapshot.savedAt > 7 * 24 * 60 * 60 * 1000) return false;

    store.products    = snapshot.products    || [];
    store.proveedores = snapshot.proveedores || [];
    store.sales       = snapshot.sales       || [];
    store.cajaHistory = snapshot.cajaHistory || [];
    store.retiros     = snapshot.retiros     || [];
    store.movimientos = snapshot.movimientos || [];
    store.ctacteMovs  = snapshot.ctacteMovs  || [];
    store.orders      = snapshot.orders      || [];
    store.combos      = snapshot.combos      || [];
    store.users       = snapshot.users       || [];
    store.nextProdId   = snapshot.nextProdId   || 8;
    store.nextProvId   = snapshot.nextProvId   || 5;
    store.nextOCId     = snapshot.nextOCId     || 1;
    store.nextUserId   = snapshot.nextUserId   || 10;
    store.nextCCId     = snapshot.nextCCId     || 1;
    store.nextRetiroId = snapshot.nextRetiroId || 1;
    store.saldoAnterior = snapshot.saldoAnterior || 0;
    store.cajaAbierta   = snapshot.cajaAbierta   || null;
    store.devoluciones  = snapshot.devoluciones  || [];
    return true;
  } catch(e) {
...
function updateConnBadge() {
  ...
  if (!online) {
    ...
    badge.title = 'Modo offline. Las ventas se guardan localmente y se sincronizan cuando vuelva la conexión.';
  } else if (queue.length > 0) {
    badge.className = 'conn-badge syncing';
    badge.innerHTML = '↑ Sincronizando...';
    badge.title = `Sincronizando ${queue.length} operación(es) pendiente(s)`;
  } else {
...
async function syncOfflineQueue() {
  ...
  for (const op of queue) {
    try {
      ...
      console.error('Sync error para operación:', op, e);
      failed.push(op);
    }
  }
  ...
  if (failed.length === 0 && queue.length > 0) {
    ...
    saveLocalData(); // Actualiza el caché local con datos confirmados
  } else if (failed.length > 0) {
    toast(`${failed.length} operación(es) pendiente(s) de sincronizar`, 'warn');
  }
}
...
function initFirebase() {
  ...
  // Habilitar persistencia offline de Firestore (fallback adicional)
...
async function saveDoc(col, id, data) {
  // Siempre actualizar datos locales primero
  saveLocalData();
  ...
  try {
    ...
    // Si falla (ej. timeout), agregar a la cola
    addToOfflineQueue({ type: 'set', col, id: String(id), data });
  }
}
...
async function loadUsersFromFirebase() {
  if (!db) {
    // Offline: usar usuarios del caché local
    const local = loadLocalData();
    if (local && store.users.length) return;
    store.users = [];
    return;
  }
  try {
    ...
    if (needsMigration) {
      console.warn('[BazarHub] Hay usuarios con contraseñas en texto plano.');
    }
  } catch(e) {
...
async function loadFromFirebase() {
  ...
  try {
    ...
    // Sincronizar pendientes que quedaron de una sesión offline anterior
...
// ===== OPERACIONES DE NEGOCIO CON PERSISTENCIA (y offline) =====
...
async function saveSale(sale, updatedProducts, newMovimientos) {
  ...
  if (!navigator.onLine || !db) {
    // Encolar todo como batch offline
    const items = [
  ...
  try {
    ...
    await batch.commit();
    saveLocalData(); // actualizar caché local post-venta
  } catch(e) {
