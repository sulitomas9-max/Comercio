...
function renderStockPage() {
  ...
  const cardVal = document.getElementById('card-st-val');
  if (store.currentUser?.role === 'admin') {
    document.getElementById('st-val').textContent = formatMoney(valTotal);
    if (cardVal) cardVal.style.display = '';
  } else {
    if (cardVal) cardVal.style.display = 'none';
  }
  renderStock('');
}
...
function renderProducts() {
  ...
  // Mostrar botón "Nuevo producto" también para cajeras
...
function openProdModal(id = null) {
  ...
  if (costRow) costRow.style.display = isAdmin ? '' : 'none';
  const provRow = document.getElementById('f-prov-row');
  if (provRow) provRow.style.display = isAdmin ? '' : 'none';
...
async function saveProd() {
  ...
  const data = {
    ...
    // Cajeras no pueden cambiar el costo; si editProdId existe y no es admin, preservar el costo original
    cost:          isAdmin ? (parseFloat(document.getElementById('f-cost').value) || 0)
                           : (store.products.find(p => p.id === store.editProdId)?.cost || 0),
  ...
  renderProducts();
  toast(store.editProdId ? 'Producto guardado' : 'Producto creado');
}
