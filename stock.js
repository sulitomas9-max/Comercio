/**
 * stock.js
 * Maneja: productos, ajuste de stock, movimientos, proveedores,
 * cuenta corriente y órdenes de compra.
...
function renderMovimientos() {
  ...
  sel.innerHTML = '<option value="">Todos los productos</option>' +
    store.products.map(p =>
      `<option value="${p.id}" ${prev == p.id ? 'selected' : ''}>${p.name}</option>`
    ).join('');
  sel.value = prev;

  const fp       = sel.value;
  const ft       = document.getElementById('mov-ftype').value;
  const filtered = store.movimientos.filter(m =>
    (!fp || m.prodId == fp) && (!ft || m.tipo === ft)
  );
...
function openMovModal(id = null) {
  ...
  document.getElementById('mov-prod').innerHTML =
    store.products.map(p => `<option value="${p.id}">${p.name}</option>`).join('');
...
async function saveMov() {
  ...
  if (store.editMovId) {
    ...
    if (mov) {
      Object.assign(mov, { prodId, prodName: prod?.name || '', tipo, cantidad: cant, motivo, fecha });
    ...
    const newMov = {
      ...
      stockDespues: prod ? prod.stock + cant : cant,
      motivo,
...
async function saveProd() {
  ...
  if (store.products.find(p => p.code === code && p.id !== store.editProdId)) {
    showMsg('msg-prod', 'Código ya existe', 'err');
    return;
  }
...
function renderProveedores() {
  const deudaTotal = store.ctacteMovs.reduce(
    (s, m) => m.type === 'deuda' ? s + m.monto : s - m.monto, 0
  );
  ...
  document.getElementById('prov-list').innerHTML = store.proveedores.map(pv => {
    ...
    const pvDeuda = store.ctacteMovs
      .filter(m => m.provId === pv.id)
      .reduce((s, m) => m.type === 'deuda' ? s + m.monto : s - m.monto, 0);
    ...
    return `
      <div class="prov-card">
        <div class="prov-head">
          <div>
            <div class="prov-nm">${pv.name}</div>
            <div class="prov-cat">${pv.cat}</div>
          </div>
...
async function saveProv() {
  ...
  if (store.editProvId) {
    ...
    await saveProveedor(pv);
  } else {
    const newPv = { id: store.nextProvId++, ...data };
    store.proveedores.push(newPv);
    await saveProveedor(newPv);
  }
...
async function delProv(id) {
  ...
  // Desasignar proveedor de los productos
...
function renderCtaCte() {
  ...
  // Cards de saldo por proveedor
  document.getElementById('cc-cards').innerHTML = store.proveedores
    .filter(pv => !fp || pv.id == fp)
    .map(pv => {
      const saldo = store.ctacteMovs
        .filter(m => m.provId === pv.id)
        .reduce((s, m) => m.type === 'deuda' ? s + m.monto : s - m.monto, 0);
      return `
        <div class="card">
          <div class="card-lbl">${pv.name}</div>
          <div class="card-val ${saldo > 0 ? 'r' : 'g'}" style="font-size:15px">
            ${saldo > 0 ? 'Debe ' + formatMoney(saldo) : 'Al día'}
          </div>
        </div>`;
    }).join('');
  ...
  tb.innerHTML = filtered.map(m => {
    ...
    const saldoColor = saldoAcum > 0 ? 'var(--red)' : 'var(--accent)';
    return `
      <tr>
        <td style="font-size:12px;color:var(--txt2);white-space:nowrap">${m.fecha}</td>
        ...
        <td style="font-weight:700;color:${saldoColor}">${saldoAcum > 0 ? formatMoney(saldoAcum) : '$0'}</td>
      </tr>`;
  }).join('');
}

function openPagoModal(provId = null) {
  document.getElementById('pago-prov').innerHTML =
    store.proveedores.map(v => `<option value="${v.id}">${v.name}</option>`).join('');
...
function renderOrdenes() {
  ...
  tb.innerHTML = [...store.orders].reverse().map(o => {
    ...
    const actions = o.status === 'pendiente' ? `
      <div style="display:flex;gap:5px">
        <button class="btn sm pri" onclick="receiveOC(${o.id})">Recibir</button>
        <button class="btn sm red" onclick="cancelOC(${o.id})">Cancelar</button>
      </div>` : '—';
    return `
      <tr>
        <td style="font-weight:700">#${o.id}</td>
        <td style="font-size:12px;color:var(--txt2)">${o.date}</td>
        <td style="font-weight:500">${pv ? pv.name : '-'}</td>
...
function addOCItem() {
  store.ocItems.push({ prodId: '', qty: 1, cost: 0, name: '' });
  renderOCItems();
}

function getOCProds() {
  return store.products.filter(p => p.provId === parseInt(document.getElementById('oc-prov').value));
}
...
function renderOCItems() {
  ...
  document.getElementById('oc-total').textContent =
    formatMoney(store.ocItems.reduce((s, i) => s + i.cost * i.qty, 0));
}

function setOCProd(index, val) {
  const prod = store.products.find(p => p.id == parseInt(val));
  store.ocItems[index].prodId = val;
  store.ocItems[index].cost   = prod ? prod.cost : 0;
  store.ocItems[index].name   = prod ? prod.name : '';
  renderOCItems();
}

function setOCQty(index, val) {
  store.ocItems[index].qty = parseInt(val) || 1;
  renderOCItems();
}

function removeOCItem(index) {
  store.ocItems.splice(index, 1);
  renderOCItems();
}
...
async function saveOC() {
  ...
  const ctaMov = {
    ...
    type:     'deuda',
    monto:    total,
    concepto: `Orden de compra #${order.id}`,
    fecha:    new Date().toLocaleString('es-AR'),
  };
...
async function receiveOC(id) {
  ...
  order.items.forEach(item => {
    ...
    if (prod) {
      const prevStock = prod.stock;
      prod.stock += item.qty;
      const mov = registrarMovimiento(prod.id, 'entrada', item.qty, prevStock, prod.stock, 'Orden compra #' + id);
  ...
  renderOrdenes();
  toast('Mercadería recibida. Stock actualizado.');
}
