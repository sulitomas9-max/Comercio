...
async function leerPDF(file) {
  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    // Facturas Berkma usan fuentes Identity-H: pdf.js emite cada carácter
    // como un item separado. Hay que agrupar chars por proximidad en X
    // para reconstruir palabras, y luego agrupar palabras por Y para líneas.
    //
    // Algoritmo:
    //  1. Recolectar todos los items con { str, x, y, endX }
    //  2. Agrupar por Y (±3px) → líneas
    //  3. Dentro de cada línea, ordenar por X y juntar chars con gap < 4px
    //     Si gap ≥ 4px → separador de palabra (espacio)
    ...
    for (const item of content.items) {
      ...
      allItems.push({ str: item.str, x, y, endX });
    }
    ...
    for (const it of allItems) {
      ...
      for (const [ly] of lineMap) {
        if (Math.abs(ly - it.y) <= 3) { found = ly; break; }
      }
      const key = found !== null ? found : it.y;
    ...
    for (const y of sortedYs) {
      ...
      for (const it of items) {
        if (prevEndX !== null) {
          ...
          // Gap ≥ 4px → nueva palabra; gap < 0 → probable overlap/kern, ignorar
          if (gap >= 4) line += ' ';
        }
        line    += it.str;
        prevEndX = it.endX;
